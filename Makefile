...
help:
	...
	@echo "  make db      - 스키마와 테스트 데이터가 존재하는 db 컨테이너를 생성합니다. api를 별도로 실행하고 싶을 때 사용합니다."
	@echo "  make emptydb - 스키마만 존재하는 db 컨테이너를 생성합니다. batch를 별도로 실행하고 싶을 때 사용합니다."
...
up:
	...
	@echo "⚠️ Note: db 컨테이너는 테스트 데이터를 제공하므로, batch 모듈을 실행할 필요가 없습니다."
	@echo "⚠️ Note: batch 컨테이너는 보안상의 이유로 제공되지 않습니다. 로컬에서 BatchApplication.java를 실행하고 싶으시다면, Naver JWT token을 발급받아 .env.example에 추가해주세요."
	@echo "⚠️ Note: api 컨테이너는 사전에 빌드된 이미지를 기반으로 작동합니다. 변경사항을 반영하고 싶으시다면 api 컨테이너를 중지하고 별도로 ApiApplication.java를 실행해 주세요."
